...
| 병목 지점 | 원인 | 개선 방안 |
|---|---|---|
| 해시맵 resize 지연 | 10만 건 쌓이는 동안 resize가 여러 번(약 14회) 발생, 매번 전체 재배치 | 초기 용량을 크게 잡거나, 한 번에 다 안 옮기고 여러 번에 나눠 옮기는 "점진적 리해싱" |
| `KEYS` 명령어 | 전체 데이터를 다 훑는 연산이라 데이터 많아질수록 느려짐 | 한 번에 일부만 보여주는 `SCAN` 방식으로 교체 (실제 Redis도 이렇게 씀) |
| 데이터가 한 곳에 다 몰림 | 지금 구조는 해시맵/DLL/힙이 하나뿐이라 다 처리 못하고 느려짐 | **샤딩**: key를 여러 조각으로 나눠서 여러 개로 쪼개 저장 |
| CPU 병렬 처리 안 됨 | 파이썬 특성상 스레드 늘려도 계산 자체는 한 번에 하나씩만 됨 | 스레드 말고 **프로세스**를 여러 개 띄워서 진짜 동시 처리 |
| 명령어 입출력이 순서대로만 처리됨 | 지금은 한 명령어 끝나야 다음 명령어 받음 (여러 명이 동시 접속하면 줄서서 기다려야 함) | 비동기 처리 방식으로 바꿔서 여러 요청 동시에 받기 |
 
**샤딩(데이터 나눠 담기)이 뭐냐면**
 
10만 개를 해시맵 하나에 다 넣지 말고, `key`를 기준으로 여러 개의 저장소(샤드)에 나눠 담는 거야. 예를 들어 샤드 4개면 `hash(key) % 4`로 어느 샤드에 넣을지 정함.
 
- **좋은 점**: 샤드 하나가 담당하는 데이터가 1/4로 줄어드니까, resize 같은 무거운 작업도 그만큼 가벼워짐
- **불편한 점**: `KEYS`나 `DBSIZE`처럼 "전체"를 봐야 하는 명령어는 샤드 4개한테 각각 물어보고 합쳐야 함. 반면 `SET`/`GET`처럼 key 하나만 다루는 명령어는 오히려 더 빨라짐

**병렬 처리가 왜 스레드만으론 안 되냐면**
 
파이썬은 스레드를 여러 개 만들어도 실제 계산(CPU 작업)은 한 번에 하나씩만 처리합니다(GIL이라는 제약 때문). 그래서 진짜 동시 처리를 하려면 **샤드마다 별도의 프로세스를 띄워서** 각자 독립적으로 돌게 만들어야 합니다. 이게 실제 Redis가 여러 대의 서버로 확장하는 방식과 같은 원리입니다.
 
**입출력(I/O) 병목이 뭐냐면**
 
지금은 명령어 하나 처리 끝나야 다음 명령어를 받을 수 있는 구조야. 사람 한 명이 쓰는 CLI에선 문제없는데, 여러 사람이 동시에 접속하는 서버로 바꾸면 이 방식은 병목이 돼. 개선하려면:
- 여러 요청을 동시에 기다릴 수 있는 **비동기 처리 방식**(파이썬이면 `asyncio`)으로 바꾸기
- 명령어를 하나씩 말고 **여러 개를 한 번에 모아서** 처리하기
